    ...
    while brain.timer.time(MSEC) - start_time < timeout:
      ...
      ##      print(bdist.object_distance(), fdist.object_distance())
        ...
        for motor in [motorLeft_1, motorLeft_2, motorLeft_3, motorRight_1, motorRight_2, motorRight_3]:
            motor.stop()
    if debug:
        print("drive", target_distance,error, motor_voltage)

def PIDrotate(target_degrees: float, max_voltage: float = 10,
              acceleration: float = 0.9, inertia: float = 0.1, drag: float = 0.02,
    ...
    while brain.timer.time(MSEC) - start_time < timeout:
        ...
        for motor in [motorLeft_1, motorLeft_2, motorLeft_3, motorRight_1, motorRight_2, motorRight_3]:
            motor.stop()
    if debug:
        print("turn", target_degrees, error, motor_voltage)
...
class Action: #One unit of action run during auton
    ...
    def execute(self):
        ...
        if self.action.upper() in ["M", "MOVE"]:
            ...
            PIDrotate(self.amount, max_voltage=self.max_voltage, timeout=self.timeout, debug=DEBUG)
        elif self.action.upper() in ["W", "WAIT"]:
            sleep(self.amount)
        for motor in motors:
            motor.stop()
...
r_auton = [
    Action("M", 706, max_voltage=7, intake=INTAKE),
    Action("W", 200),
    Action("T", -60, max_voltage=10),
    ActionDeprecated("F", 450, 25),
    Action("W", 70),
    Action("W", 1400, intake=LOWER),
    Action("W", 100),
    Action("T", -10, max_voltage=10, pistons=(0, 1)),
    ActionDeprecated("B", 1920, 40),
    Action("T", -119, max_voltage=10, pistons=(1, 1)),
    ActionDeprecated("F", 670, 35, intake=INTAKE),
    Action("W", 100),
    ActionDeprecated("B", 30, 10),
    ActionDeprecated("F", 30, 25, intake=INTAKE),
    ActionDeprecated("B", 30, 25, intake=INTAKE),
    ActionDeprecated("F", 30, 25, intake=INTAKE),
    ActionDeprecated("B", 30, 25, intake=INTAKE),
    Action("W", 500, intake=INTAKE),
    Action("M", -250, max_voltage=10, sensor=fdist, intake=INTAKE),
    Action("W", 20),
    Action("T", 10, max_voltage=10),
    Action("W", 20),
    Action("M", -400, max_voltage=12, sensor=fdist, pistons=(1, 1)),
    Action("W", 1000, intake=UPPER),
    Action("M", 200, max_voltage=12, sensor=fdist),
    Action("T", 110, max_voltage=12, pistons=(0,1)),
    ActionDeprecated("F", 420, 100),
    Action("T", 50, max_voltage=12),
    ActionDeprecated("F", 1000, 90),
]

l_auton = [
    #Action("T", 20, max_voltage=5),
    Action("M", 300, max_voltage=7, intake=INTAKE),
    ActionDeprecated("F", 950, 35, intake=INTAKE),
    Action("W", 100),
    ActionDeprecated("B", 350, 15),
    Action("T", -101, max_voltage=10),
    ActionDeprecated("B", 785, 20),
    Action("W", 1200, intake=CENTER, pistons=(0, 1)),
    ActionDeprecated("F", 60, 25, pistons=(0, 1)),
    Action("T", 12, max_voltage=10),
    ActionDeprecated("F", 1800, 25, pistons=(1, 0)),
    Action("T", -51, max_voltage=10),
    ActionDeprecated("F", 810, 45, intake=INTAKE),
    Action("W", 1000, intake=INTAKE),
    Action("M", -220, max_voltage=12, sensor=fdist),
    Action("M", -500, max_voltage=12, sensor=fdist, intake=INTAKE,  pistons=(1, 1)),
    Action("W", 1500, intake=UPPER),
    Action("M", 200, max_voltage=12, sensor=fdist),
    Action("T", 107, max_voltage=12, pistons=(0,1)),
    ActionDeprecated("F", 420, 100),
    Action("T", 60, max_voltage=12),
    ActionDeprecated("F", 1000, 90),
]
