...
SIDE = "SKILLS" #"LEFT", "RIGHT", or "SKILLS"
...
class Action: #One unit of action run during auton
    def __init__(self, action, degrees, velocity=50, special=0, stopIntake=True):
        self.action = action
...
SKILLS_auton_actions = [
    ...
    Action("L", 600, 15, 5),
    Action("FT", 1000, 30),
    #the following is shuffled to prevent overheating the motors, which was an issue during testing when the intake and flapper were used in quick succession
    #The top was an ai tab thing
    #The following actions are shuffling back and forth to get the stuff out of the match loader 
    Action("FT", 200, 25, 1, False),
    Action("W", 300, 0, 1, False),
    Action("B", 50, 25, 1, False),
    Action("FT", 200, 25, 1, False),
    Action("W", 300, 0, 1, False),
    Action("B", 50, 25, 1, False),
    Action("FT", 200, 25, 1, False),
